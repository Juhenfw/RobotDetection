...
# Tambahkan struktur data untuk menyimpan waktu masuk objek ke checkpoint
object_entry_time = {area: {} for area in checkpoints}

# Tambahkan struktur data untuk menyimpan timer aktif
...
while cap.isOpened():
            ...
            for area_name, area_coords in checkpoints.items():
                if is_in_area([x1, y1, x2, y2], area_coords):
                    ...
                    if object_id not in object_in_checkpoint[area_name]:
                        ...
                        object_entry_time[area_name][object_id] = current_time
                    ...
                    if object_id in object_entry_time[area_name]:
                        time_in_area = current_time - object_entry_time[area_name][object_id]
    ...
    for area_name in checkpoints:
        ...
        for object_id in exited_objects:
            if object_id in object_entry_time[area_name] and active_timers[area_name].get(object_id, False):
                ...
                print(f"Objek {class_name} keluar dari {area_name} setelah {duration:.2f} detik")
    ...
    # Tampilkan frame
    cv2.imshow("Deteksi Objek dengan Checkpoint dan Timer", frame)
